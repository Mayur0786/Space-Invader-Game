...
enemyX_change = []
enemyY_change = []
enemy_speed = 1  # Adjust the speed here
num_of_enemies = 2
...
game_over_sound = mixer.Sound("Moye Moye1.wav")


def show_score(x, y):
    score = font.render(f" {user_name}'s Score: {user_scores[user_name]}", True, (255, 255, 255))
...
def add_new_enemies():
    ...
    enemyX.append(random.randint(0, 736))
    enemyY.append(random.randint(50, 150))
    enemyX_change.append(enemy_speed)  # Set the initial speed for each new enemy
    enemyY_change.append(40)
    ...
    for i in range(num_of_enemies):
        ...
        if collision:
            ...
            if user_scores[user_name] % 10 == 0 and user_scores[user_name] != 0:
                enemy_speed += 1
                add_new_enemies()  # Add new enemies when the score is a multiple of 10
